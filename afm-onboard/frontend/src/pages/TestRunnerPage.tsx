        ...
        <div className="flex-1 min-w-0">
          ...
          {!isPreview && (!attemptId || items.length === 0) && (
            <div className="mb-4 rounded border border-amber-500/40 bg-amber-50 text-amber-700 p-3">
              {items.length === 0 ? 'Тест пока не содержит вопросов. Обратитесь к администратору.' : lessonId ? 'Чтобы приступить к тесту урока, завершите урок и все предыдущие уроки модуля.' : 'Чтобы приступить к тесту модуля, завершите все уроки модуля.'}
            </div>
          )}
