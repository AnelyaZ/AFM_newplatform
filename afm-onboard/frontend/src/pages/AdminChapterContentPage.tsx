...
export default function AdminChapterContentPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/chapters/${id}/contents`);
      setBlocks(data);
    } finally {
      setLoading(false);
    }
  ...
  const save = async () => {
    await api.post(`/chapters/${id}/contents`, {
      blocks: blocks.map(({ id: _ignore, ...b }) => b),
    });
    push({ type: 'success', title: 'Контент сохранён' });
    await load();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col items-start justify-between gap-2 md:flex-row md:items-center">
        <h2 className="text-2xl font-semibold">Материалы модуля</h2>
                ...
                <div className="mb-3 flex flex-col items-start justify-between gap-3 md:flex-row md:items-center">
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-3 w-full md:w-auto">
                    <Select
                      label="Тип блока"
                      value={b.blockType}
                      onChange={(e) => setBlocks((arr) => arr.map((x, i) => i === idx ? { ...x, blockType: e.target.value as Block['blockType'] } : x))}
                      options={[
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {b.mediaKey && uploadingIdx !== idx && (<span className="truncate text-sm text-white/70">{b.mediaKey}</span>)}
                      </div>
                    </div>
                  </div>
                )}
